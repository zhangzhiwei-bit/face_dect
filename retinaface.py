...
net = Net1(mobilenet)
net.load_state_dict(torch.load('model_data/test_mask.pth',map_location=torch.device('cpu')))
def dect_mask(net,frame):
    reimg=cv2.resize(frame,(224,224))
    reimg=reimg.astype('float32')
    test_img=torch.from_numpy(reimg.transpose((2,0,1))).unsqueeze(0)
    return int(net(test_img).argmax(1))
...
class Retinaface(object):
    _defaults = {
        ...
        "facenet_model_path": 'model_data/test2.pth',
        # -----------------------------------#
        #   可选facenet_backbone有
    ...
    def generate(self):
        ...
        state_dict = torch.load(self.facenet_model_path, map_location='cpu')
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image,dect):
        # 绘制人脸框
        image = np.array(image, np.float32)
        ...
        for i, b in enumerate(boxes_conf_landms):
            ...
            cv2.putText(old_image, dect, (b[0]+65 , b[3] - 10), font, 0.55, (255, 255, 255), 1)
            # --------------------------------------------------------------#
